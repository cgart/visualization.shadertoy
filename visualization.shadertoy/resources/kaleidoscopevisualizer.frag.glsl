...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	for ( int i = 0; i < 128; i++ ) {
		vec4 noise  = texture2D( iChannel1, vec2( float( i ) + 0.5, 0.5 ) / 256.0 );
		vec4 noise2 = texture2D( iChannel1, vec2( float( i ) + 0.5, 9.5 ) / 256.0 );


		//sound loudness to intensity
		float velintensity = texture2D(iChannel0, vec2(1.0+uv.x,0)).r;
		...
		//sound loudness to intensity
		float intensity = texture2D(iChannel0, vec2(pos.y,0)).r;
